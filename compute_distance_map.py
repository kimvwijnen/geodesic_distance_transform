...
def checkdots_anydim(dist, seedarray):
    for s in seedarray:
        assert (dist[tuple(s)] == 0.0), \
            '***BUG***! distance of dot is larger than 0! Distance is: ' + str(dist[tuple(s)])
...
def calc_gdm(alpha, p_val, q_val, scaling_factor, squared_dist_f, i):
    ...
    dist_pq = alpha * np.sqrt(np.square(p_val - q_val) * scaling_factor + squared_dist_f[i])
    return dist_pq
        ...
        for h in looph:
            for w in loopw:
                ...
                for i in range(k_p['h'].shape[0]):
                    ...
                    dist_pq = func_dist(alpha, p_val, q_val, scaling_factor, squared_dist, i)
...
def get_3d_kernel(backward=False):
    if backward:
        ...
        squared_dist = np.array([2.0, 3.0, 2.0, 3.0, 1.0, 2.0, 1.0, 2.0, 1.0, 2.0, 3.0, 2.0, 3.0])
        ...
        squared_dist = np.array([3.0, 2.0, 3.0, 2.0, 1.0, 2.0, 1.0, 2.0, 1.0, 3.0, 2.0, 3.0, 2.0])
...
def compute_dm_rasterscan(im, seeds, its=8, dist_type='', scaling_factor=1):
    ...
    assert seeds.shape[1] == len(img.shape), \
        "ERROR, intensity image " + str(len(img.shape)) + "D but seed points " + str(seeds.shape[1]) + "D, should be the same!"
    ...
    for it in range(its):
        ...
        distance = pass_func(img, distance, k_f, func_dist, squared_dist_f, scaling_factor)

        # backward scan
        k_b, squared_dist_b = get_kernel(backward=True)
        distance = pass_func(img, distance, k_b, func_dist, squared_dist_b, scaling_factor, backward=True)
...
if __name__ == '__main__':
    im = 255 * np.random.random([200,300])
    seeds = np.random.random(im.shape)
    seeds = np.transpose(np.where(seeds > 0.99))
    dist_map = compute_dm_rasterscan(im, seeds, its=2, dist_type='geodesic')
